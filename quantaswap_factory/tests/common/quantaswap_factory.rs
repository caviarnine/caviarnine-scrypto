...
pub fn build_manifest(
    quantaswap_factory_package: PackageAddress,
...
pub fn set_owner_rule(rule: AccessRule, with_proof: bool, vars: &mut Vars) -> TransactionReceipt {
    let manifest = if with_proof {
        ManifestBuilder::new()
            ...
            .set_owner_role(vars.quantaswap_factory_component, rule)
            .build()
    } else {
        ManifestBuilder::new()
...
pub fn set_role_rule(role: String, rule: AccessRule, with_proof: bool, vars: &mut Vars) -> TransactionReceipt {
    let manifest = if with_proof {
        ManifestBuilder::new()
            .create_proof_from_account_of_amount(vars.admin_account_component, vars.admin_badge, dec!(1))
            .set_role(vars.quantaswap_factory_component, ObjectModuleId::Main, RoleKey { key: role }, rule)
            .build()
    } else {
        ManifestBuilder::new()
            // .create_proof_from_account(vars.admin_account_component, vars.admin_badge)
...
pub fn get_pool_count(vars: &mut Vars) -> u64 {
    let manifest = ManifestBuilder::new()
        .call_method(
            vars.quantaswap_factory_component,
            "get_pool_count",
...
pub fn get_pools(start: Option<u64>, end: Option<u64>, vars: &mut Vars) -> Vec<ComponentAddress> {
    let manifest = ManifestBuilder::new()
        .call_method(
            vars.quantaswap_factory_component,
            "get_pools",
...
pub fn get_pool_pair(pool_address: ComponentAddress, vars: &mut Vars) -> Option<(ResourceAddress, ResourceAddress)> {
    let manifest = ManifestBuilder::new()
        .call_method(
            vars.quantaswap_factory_component,
            "get_pool_pair",
            manifest_args!(pool_address))
        .build();
...
pub fn get_pools_by_pair(token_x_address: ResourceAddress, token_y_address: ResourceAddress, start: Option<u64>, end: Option<u64>, vars: &mut Vars) -> Vec<ComponentAddress> {
    let manifest = ManifestBuilder::new()
        .call_method(
            vars.quantaswap_factory_component,
            "get_pools_by_pair",
...
pub fn new_pool(token_x: ResourceAddress, token_y: ResourceAddress, bin_span: u32, vars: &mut Vars) -> TransactionReceipt {
    let manifest = ManifestBuilder::new()
        .call_method(
            vars.quantaswap_factory_component,
            "new_pool",
            manifest_args!(token_x, token_y, bin_span, None::<ManifestAddressReservation>))
        .build();
