...
pub fn get_redemption_value(liquidity_receipt_id: NonFungibleLocalId, vars: &mut Vars) -> (Decimal, Decimal) {
    let manifest = ManifestBuilder::new()
    .call_method(
        vars.quantaswap_component,
    ...
    // println!("{:?}", receipt);
    receipt.expect_commit_success().output::<(Decimal, Decimal)>(1)
}

pub fn get_redemption_bin_values(liquidity_receipt_id: NonFungibleLocalId, vars: &mut Vars) -> Vec<(u32, Decimal, Decimal)> {
    let manifest = ManifestBuilder::new()
    .call_method(
        vars.quantaswap_component,
    ...
    // println!("\nGET REDEMPTION VALUE\n");
    // println!("{:?}", receipt);
    receipt.expect_commit_success().output::<Vec<(u32, Decimal, Decimal)>>(1)
}
...
pub fn burn_liquidity_receipt(liquidity_receipt_id: NonFungibleLocalId, vars: &mut Vars) -> TransactionReceipt {
...
pub fn remove_specific_liquidity(
    liquidity_receipt_id: NonFungibleLocalId, 
