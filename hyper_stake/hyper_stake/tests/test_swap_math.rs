...
fn test_virtual_amounts_max_range() {
    ...
    let lower_limit = MIN_PRICE_SQRT;
    let upper_limit = MAX_PRICE_SQRT;
...
#[test]
fn test_extremes() {
    let limit_min = MIN_PRICE_SQRT;
    let limit_min_up = limit_min * dec!(1.0005);    

    let limit_max = MAX_PRICE_SQRT;
    let limit_max_down = limit_max / dec!(1.0005);
        ...
        for real_y in &amounts {
            for upper_limit in &price_limits {
                for lower_limit in &price_limits {
                    if *upper_limit * *upper_limit <= *lower_limit * *lower_limit {
                        continue;
                    }
